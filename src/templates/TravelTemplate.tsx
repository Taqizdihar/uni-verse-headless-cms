  ...
  if (postData) {
    return (
      <div style={{ ...vars, background: 'var(--bg-color)', color: 'var(--text-main)', minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'Inter', sans-serif" }}>
        <TravelNavbar settings={settings} siteName={siteName} navPages={navPages} currentSlug={currentSlug} p={p} subdomain={subdomain} />
        <main style={{ flex: 1 }}>
          <UnifiedPostLayout postData={postData} palette={p} currentSlug={currentSlug} />
        </main>
        <TravelFooter settings={settings} siteName={siteName} footerCfg={footerCfg} p={p} />
      </div>
    );
  }
          ...
          <div className="animate-in fade-in duration-1000">
             <section style={{ height: '85vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1, padding: '0 4vw', maxWidth: '1100px' }}>
                   <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.75rem', padding: '0.6rem 2rem', background: 'var(--primary)', color: '#FFF', borderRadius: '5px', fontWeight: 900, fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.4em', marginBottom: '4rem', transform: 'rotate(-2deg)' }}>
                   ...
                   <Link to={`/preview/${subdomain}/contact`} style={{ display: 'inline-block', padding: '1.2rem 4rem', background: 'var(--secondary)', color: '#FFF', fontWeight: 900, textTransform: 'uppercase', textDecoration: 'none', letterSpacing: '0.2rem', boxShadow: '10px 10px 0 var(--text-main)', fontSize: '0.85rem' }}>
                      START JOURNEY
                   </Link>
                </div>
             ...
             <div style={{ padding: '6rem 4vw', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '4rem', background: 'var(--text-main)', color: '#FFF' }}>
                {[
                  ...
                  <div key={i} style={{ borderLeft: '2px solid var(--secondary)', paddingLeft: '2.5rem' }}>
          ...
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '12rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: '10rem', alignItems: 'center' }}>
                <div style={{ position: 'relative' }}>
                   <div style={{ position: 'absolute', top: '-4rem', left: '-4rem', width: '200px', height: '200px', background: 'var(--secondary)22', borderRadius: '50%' }} />
                   <h2 style={{ fontSize: '5rem', fontWeight: 950, color: 'var(--text-main)', lineHeight: 0.9 }}>Explorer<br />Charter.</h2>
                </div>
                <div 
        ...
        return (
          <div style={{ maxWidth: '1800px', margin: '0 auto', padding: '10rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ marginBottom: '10rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div>
                   {content.gallery_title && <h1 style={{ fontSize: '5rem', fontWeight: 950, color: 'var(--text-main)' }}>{content.gallery_title}</h1>}
             ...
             <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(500px, 1fr))', gap: '4rem' }}>
                {galleryImages.map((img: string, i: number) => (
                  <div key={i} style={{ borderRadius: '2rem', overflow: 'hidden', height: '650px', position: 'relative', border: '5px solid #F1F5F9', boxShadow: '0 40px 80px rgba(12,74,110,0.05)' }} className="group">
                     ...
                     <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, var(--text-main), transparent)', opacity: 0, transition: 'all 0.4s' }} className="group-hover:opacity-60" />
                  </div>
                ))}
             </div>
          </div>
          ...
          <div style={{ maxWidth: '1600px', margin: '0 auto', padding: '8rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '8rem', marginBottom: '12rem', background: '#F8FAFC', borderRadius: '3rem', overflow: 'hidden', border: '1px solid #F1F5F9' }}>
                <div style={{ padding: '8rem' }}>
                   <div style={{ display: 'inline-flex', padding: '1rem 3rem', background: 'var(--text-main)', color: '#FFF', fontWeight: 900, fontSize: '0.75rem', borderRadius: '5px', marginBottom: '3rem', textTransform: 'uppercase', letterSpacing: '0.3em' }}>EXPEDITION LOGS</div>
                   <h1 style={{ fontSize: '5rem', fontWeight: 950, lineHeight: 0.9, color: 'var(--text-main)', marginBottom: '3rem' }}>{pageData.title}</h1>
                  ...
                  <Link key={post.id} to={`/preview/${subdomain}/${post.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                     <article style={{ background: '#FFF', borderRadius: '2rem', overflow: 'hidden', transition: 'all 0.4s' }} className="group">
                        <div style={{ height: '350px', position: 'relative' }}>
                           ...
                           <div style={{ position: 'absolute', bottom: '2rem', left: '2rem', background: 'var(--primary)', color: '#FFF', padding: '0.5rem 1.5rem', fontWeight: 900, fontSize: '0.75rem' }}>DISCOVER</div>
                        </div>
                        <div style={{ padding: '3.5rem', border: '2px solid #F1F5F9', borderTop: 'none', borderRadius: '0 0 2rem 2rem' }}>
                           <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
                              <span style={{ fontWeight: 900, color: 'var(--secondary)', fontSize: '0.8rem', textTransform: 'uppercase' }}>{post.category}</span>
                              <span style={{ fontSize: '0.7rem', opacity: 0.3, fontWeight: 900 }}>{new Date(post.created_at).toLocaleDateString()}</span>
                           </div>
                           <h3 style={{ fontSize: '2.2rem', fontWeight: 950, color: 'var(--text-main)', marginBottom: '2.5rem', lineHeight: 1.1 }}>{post.title}</h3>
                           <p style={{ opacity: 0.5, fontSize: '1.1rem', lineHeight: 1.6, marginBottom: '3.5rem' }}>{post.excerpt || post.content?.summary}</p>
                           <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', fontWeight: 900, fontSize: '0.9rem', color: 'var(--text-main)', borderBottom: '2px solid var(--secondary)', paddingBottom: '0.5rem', width: 'fit-content' }}>
        ...
        return (
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '12rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15rem', alignItems: 'center' }}>
                <div>
                   <h1 style={{ fontSize: '6rem', fontWeight: 950, color: 'var(--text-main)', lineHeight: 0.9, marginBottom: '8rem' }}>Direct Line to<br />the Wild.</h1>
                   <div style={{ display: 'flex', flexDirection: 'column', gap: '6rem' }}>
                      {content.email && (
                        <div>
                           ...
                           <b style={{ fontSize: '2.2rem', color: 'var(--secondary)' }}>{content.email}</b>
                        </div>
                      )}
                      {content.phone && (
                        <div>
                           ...
                           <b style={{ fontSize: '2.5rem', color: 'var(--primary)', fontWeight: 900 }}>{content.phone}</b>
                        </div>
                      )}
                   </div>
                </div>
...
function TravelNavbar({ settings, siteName, navPages, currentSlug, p, subdomain }: any) {
  return (
    <nav style={{ height: '110px', padding: '0 4vw', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.85)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(30px)', borderBottom: '6px solid var(--primary)' }}>
      <Link to={`/preview/${subdomain}`} style={{ textDecoration: 'none', color: 'inherit', display: 'flex', alignItems: 'center', gap: '2rem' }}>
        <div style={{ width: '56px', height: '56px', background: 'var(--primary)', borderRadius: '5px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '10px 10px 0 var(--text-main)' }}>
        ...
        {settings?.logo_url ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
             <span style={{ fontWeight: 950, fontSize: '1.8rem', color: 'var(--text-main)', letterSpacing: '-0.04em', textTransform: 'uppercase', fontStyle: 'italic', borderBottom: '4px solid var(--secondary)', lineHeight: 0.9 }}>{siteName}</span>
             <span style={{ fontSize: '0.65rem', fontWeight: 900, color: 'var(--primary)', letterSpacing: '0.4rem', marginTop: '0.5rem' }}>GLOBAL_ACCESS</span>
          </div>
        )}
      </Link>
      <div style={{ display: 'flex', gap: '3vw', alignItems: 'center' }}>
        ...
        <Link to={`/preview/${subdomain}/contact`} style={{ padding: '1rem 3rem', background: 'var(--primary)', color: '#FFF', borderRadius: '5px', fontWeight: 900, fontSize: '0.85rem', textTransform: 'uppercase', textDecoration: 'none', boxShadow: '8px 8px 0 var(--text-main)' }}>
           ENROL
        </Link>
      </div>
    </nav>
...
function TravelFooter({ settings, siteName, footerCfg, p }: any) {
      ...
      <div style={{ maxWidth: '1800px', margin: '0 auto', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 2fr', gap: '10rem', position: 'relative', zIndex: 1 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '2rem', marginBottom: '4rem' }}>
            <div style={{ width: '40px', height: '40px', background: 'var(--primary)', borderRadius: '5px' }} />
        ...
        <div>
           <h4 style={{ fontSize: '0.8rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.5em', marginBottom: '4rem', color: 'var(--primary)' }}>Explorer Path</h4>
        ...
        <div>
           <h4 style={{ fontSize: '0.8rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.5em', marginBottom: '4rem', color: 'var(--primary)' }}>Registry</h4>
        ...
        <div>
           <h4 style={{ fontSize: '0.8rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.5em', marginBottom: '4rem', color: 'var(--primary)' }}>Signal Relay</h4>
