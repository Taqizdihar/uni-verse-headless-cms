...
export default function RestoTemplate({ 
  ...
  if (postData) {
    return (
      <div style={{ background: 'var(--bg-color)', color: 'var(--text-color)', minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'Playfair Display', serif" }}>
        <RestoNavbar settings={settings} siteName={siteName} navPages={navPages} currentSlug={currentSlug} subdomain={subdomain} />
        <main style={{ flex: 1 }}>
          <UnifiedPostLayout postData={postData} currentSlug={currentSlug} />
        </main>
        <RestoFooter settings={settings} siteName={siteName} footerCfg={footerCfg} />
      </div>
    );
  }
          ...
          <div className="animate-in fade-in duration-1000">
             <section style={{ height: '85vh', position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1, padding: '0 4vw', maxWidth: '1000px' }}>
                   ...
                   <Link to={`/preview/${subdomain}/contact`} style={{ display: 'inline-block', padding: '1.2rem 4rem', background: 'var(--primary-color)', color: '#FFF', textTransform: 'uppercase', fontWeight: 900, textDecoration: 'none', letterSpacing: '0.2em', borderRadius: '100px', fontSize: '0.8rem', boxShadow: '0 30px 60px rgba(0,0,0,0.3)' }}>
                      RESERVE TABLE
                   </Link>
                </div>
             </section>
             <div style={{ padding: '6rem 4vw', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '4rem', background: 'var(--bg-color)', borderBottom: '1px solid var(--secondary-color)11' }}>
                {[
                  ...
                  <div key={i} style={{ textAlign: 'center', padding: '3rem', border: '1px solid var(--secondary-color)11', borderRadius: '4rem' }}>
                     <item.i size={32} color="var(--primary-color)" style={{ marginBottom: '1.5rem' }} />
        ...
        return (
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '12rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: '10rem', alignItems: 'center' }}>
                <div style={{ padding: '6rem', borderRadius: '8rem', background: 'var(--secondary-color)', color: 'var(--bg-color)', position: 'relative' }}>
      ...
      case 'news':
        return (
          <div style={{ maxWidth: '1600px', margin: '0 auto', padding: '8rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '8rem', marginBottom: '12rem', background: '#FFF', borderRadius: '6rem', overflow: 'hidden', border: '1px solid var(--secondary-color)11' }}>
             ...
             <div style={{ display: 'flex', alignItems: 'center', gap: '3rem', marginBottom: '8rem' }}>
                ...
                <div style={{ height: '1px', flex: 1, background: 'var(--secondary-color)22' }} />
             </div>
        ...
        return (
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '12rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15rem', alignItems: 'center' }}>
                <div>
                   <h1 style={{ fontSize: '6rem', fontWeight: 300, fontStyle: 'italic', color: 'var(--secondary-color)', lineHeight: 0.9, marginBottom: '8rem' }}>Reserve your<br />experience.</h1>
                   <div style={{ display: 'flex', flexDirection: 'column', gap: '6rem' }}>
                      {content.email && (
                        <div>
                           ...
                           <b style={{ fontSize: '2.2rem', fontStyle: 'italic', color: 'var(--text-color)' }}>{content.email}</b>
                        </div>
                      )}
                      {content.phone && (
                        <div>
                           ...
                           <b style={{ fontSize: '2.5rem', color: 'var(--text-color)', fontWeight: 200 }}>{content.phone}</b>
                        </div>
                      )}
                   </div>
                </div>
                <div>
                   {mapsUrl && (
                     <div style={{ width: '100%', height: '800px', borderRadius: '8rem', overflow: 'hidden', border: '2px solid var(--secondary-color)11', boxShadow: '0 50px 100px rgba(120,53,15,0.05)' }}>
...
function RestoNavbar({ settings, siteName, navPages, currentSlug, subdomain }: any) {
  return (
    <nav style={{ height: '120px', padding: '0 6vw', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,250,237,0.85)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--secondary-color)11' }}>
      <Link to={`/preview/${subdomain}`} style={{ textDecoration: 'none', color: 'inherit', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        {settings?.logo_url ? (
          ...
          <>
             <span style={{ fontWeight: 300, fontSize: '2.2rem', color: 'var(--text-color)', letterSpacing: '0.3em', textTransform: 'uppercase', fontStyle: 'italic' }}>{siteName}</span>
             <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', width: '100%', marginTop: '0.5rem' }}>
                <div style={{ flex: 1, height: '1px', background: 'var(--primary-color)' }} />
                <UtensilsCrossed size={12} color="var(--primary-color)" />
                <div style={{ flex: 1, height: '1px', background: 'var(--primary-color)' }} />
             </div>
          </>
        )}
      ...
      <div style={{ display: 'flex', gap: '4vw', alignItems: 'center' }}>
        ...
        <Link to={`/preview/${subdomain}/contact`} style={{ padding: '0.8rem 2.8rem', background: 'var(--secondary-color)', color: 'var(--bg-color)', borderRadius: '100px', fontWeight: 900, fontSize: '0.75rem', textTransform: 'uppercase', textDecoration: 'none', letterSpacing: '0.2em', boxShadow: '0 15px 30px var(--secondary-color)33' }}>
           RESERVE
        </Link>
      </div>
    </nav>
...
function RestoFooter({ settings, siteName, footerCfg }: any) {
  const contact = footerCfg.contact_info || {};
  return (
    <footer style={{ background: '#FFF', borderTop: '4px solid var(--primary-color)', padding: '15rem 4vw 6rem' }}>
      <div style={{ maxWidth: '1800px', margin: '0 auto', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 2fr', gap: '10rem' }}>
        <div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', marginBottom: '4rem' }}>
            <span style={{ fontWeight: 300, fontSize: '2.2rem', color: 'var(--text-color)', letterSpacing: '0.3em', textTransform: 'uppercase', fontStyle: 'italic' }}>{siteName}</span>
            <div style={{ width: '100px', height: '1px', background: 'var(--primary-color)', marginTop: '0.5rem' }} />
          </div>
          <p style={{ opacity: 0.4, lineHeight: 2, fontSize: '1rem', marginBottom: '5rem', maxWidth: '450px', fontStyle: 'italic', color: 'var(--text-color)' }}>{footerCfg.short_description || footerCfg.footer_description}</p>
          <div style={{ display: 'flex', gap: '2.5rem' }}>
             {(footerCfg.social_links || []).map((social: any, i: number) => (
                <a key={i} href={social.url} target="_blank" rel="noreferrer" style={{ color: 'var(--secondary-color)', opacity: 0.2, transition: 'all 0.4s' }} onMouseOver={e => { e.currentTarget.style.opacity = '1'; e.currentTarget.style.color = 'var(--primary-color)'; e.currentTarget.style.transform = 'translateY(-10px)'; }} onMouseOut={e => { e.currentTarget.style.opacity = '0.2'; e.currentTarget.style.color = 'var(--secondary-color)'; e.currentTarget.style.transform = 'none'; }}>
        ...
        <div>
           ...
           <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem', fontSize: '1.2rem', fontStyle: 'italic' }}>
              {contact.phone && <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}><Phone size={22} color="var(--primary-color)" /> <b>{contact.phone}</b></div>}
              {contact.whatsapp && <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}><MessageCircle size={22} color="var(--primary-color)" /> <b>CHANNEL</b></div>}
              ...
              {footerCfg.location_embed_link && (
                <div style={{ width: '100%', height: '240px', borderRadius: '4rem', overflow: 'hidden', marginTop: '2rem', border: '1px solid var(--secondary-color)11', filter: 'sepia(0.3) saturate(1.2)' }}>
      ...
      <div style={{ maxWidth: '1800px', margin: '12rem auto 0', paddingTop: '4rem', borderTop: '1px solid var(--secondary-color)11', fontSize: '0.85rem', opacity: 0.1, textAlign: 'center', fontWeight: 900, letterSpacing: '1.2em', textTransform: 'uppercase' }}>
