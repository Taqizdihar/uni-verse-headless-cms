  ...
  if (postData) {
    return (
      <div style={{ ...vars, background: 'var(--bg-color)', color: 'var(--text-main)', minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'Outfit', sans-serif" }}>
        <NGOTopBar />
        <NGONavbar settings={settings} siteName={siteName} navPages={navPages} currentSlug={currentSlug} p={p} subdomain={subdomain} />
        <main style={{ flex: 1 }}>
          <UnifiedPostLayout postData={postData} palette={p} currentSlug={currentSlug} />
        </main>
        <NGOFooter settings={settings} siteName={siteName} footerCfg={footerCfg} p={p} />
      </div>
    );
  }
  ...
  const renderContent = () => {
    switch (pageType) {
        ...
        return (
          <div className="animate-in fade-in duration-1000">
             <section style={{ padding: '10rem 4vw', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '8rem', alignItems: 'center', background: 'linear-gradient(to bottom right, #F0FDF4, #FFF)' }}>
                <div>
                   <div style={{ display: 'inline-flex', alignItems: 'center', gap: '1rem', padding: '0.6rem 2rem', background: '#FFF', borderRadius: '100px', color: 'var(--primary)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.2rem', fontSize: '0.75rem', marginBottom: '3.5rem', boxShadow: '0 20px 40px rgba(0,0,0,0.05)' }}>
                      <Sparkles size={16} color="var(--secondary)" /> Power of Giving
                   </div>
                   {content.headline && (
                     <h1 style={{ fontSize: 'clamp(3.5rem, 8vw, 6.5rem)', fontWeight: 900, color: 'var(--text-main)', lineHeight: 0.9, letterSpacing: '-0.05em', marginBottom: '3.5rem', fontStyle: 'italic' }}>
                        {content.headline}
                     </h1>
                   )}
                   ...
                   <div style={{ display: 'flex', gap: '3rem', flexWrap: 'wrap' }}>
                      <Link to={`/preview/${subdomain}/contact`} style={{ padding: '1.5rem 4rem', background: 'var(--primary)', color: '#FFF', borderRadius: '3rem', fontWeight: 900, fontSize: '1rem', textTransform: 'uppercase', textDecoration: 'none', boxShadow: '0 25px 50px -12px var(--primary)44' }}>
                         JOIN THE MISSION
                      </Link>
                      <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                         <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: '#FFF', border: '5px solid var(--secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><HandHeart size={32} color="var(--secondary)" /></div>
                ...
                {content.hero_image && (
                  <div style={{ position: 'relative' }}>
                     <div style={{ position: 'absolute', inset: '-3rem', border: '2px dashed var(--primary)33', borderRadius: '50%', animation: 'spin-slow 60s linear infinite' }} />
             ...
             <div style={{ padding: '6rem 4vw', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '4rem', background: '#FFF' }}>
                {[
                  ...
                  <div key={i} style={{ textAlign: 'center', padding: '3rem', borderRadius: '3rem', background: 'var(--bg-color)', border: '1px solid #F0FDF4' }}>
                     {item.i && <item.i size={48} color="var(--primary)" style={{ marginBottom: '2rem' }} />}
        ...
        return (
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '10rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '10rem', alignItems: 'center' }}>
                <div style={{ padding: '6rem', borderRadius: '5rem', background: 'var(--text-main)', color: '#FFF', position: 'relative', overflow: 'hidden' }}>
        ...
        return (
          <div style={{ maxWidth: '1600px', margin: '0 auto', padding: '8rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ textAlign: 'center', marginBottom: '8rem' }}>
                {content.gallery_title && <h1 style={{ fontSize: '5rem', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-0.05em' }}>{content.gallery_title}</h1>}
      ...
      case 'news':
        return (
          <div style={{ maxWidth: '1600px', margin: '0 auto', padding: '8rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: '8rem', marginBottom: '10rem', background: 'var(--text-main)', color: '#FFF', borderRadius: '5rem', overflow: 'hidden' }}>
                <div style={{ padding: '6rem' }}>
                   <div style={{ display: 'inline-flex', padding: '0.4rem 1.5rem', background: 'var(--primary)', color: '#FFF', fontWeight: 900, fontSize: '0.75rem', borderRadius: '100px', marginBottom: '3rem', textTransform: 'uppercase', letterSpacing: '0.2em' }}>IMPACT_UPDATES</div>
             ...
             <div style={{ display: 'flex', alignItems: 'center', gap: '3rem', marginBottom: '6rem' }}>
                ...
                <div style={{ height: '4px', flex: 1, background: 'var(--primary)33', borderRadius: '10px' }} />
             </div>
        ...
        return (
          <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '10rem 4vw' }} className="animate-in fade-in duration-700">
             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.2fr', gap: '10rem', alignItems: 'center' }}>
                <div>
                   <h1 style={{ fontSize: '5.5rem', fontWeight: 900, color: 'var(--text-main)', lineHeight: 0.85, letterSpacing: '-0.05em', marginBottom: '6rem', fontStyle: 'italic' }}>Join the Mission.</h1>
                      ...
                      {content.phone && (
                        <div style={{ display: 'flex', gap: '2.5rem', alignItems: 'center' }}>
                           <div style={{ width: '72px', height: '72px', borderRadius: '2rem', background: '#FEF3C7', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <Phone size={32} color="var(--secondary)" />
                           </div>
                           <div>
                ...
                <div style={{ padding: '1rem', borderRadius: '5rem', border: '2px dashed var(--primary)33' }}>
                   {mapsUrl && (
...
function NGOTopBar() {
  return (
    <div style={{ background: 'var(--text-main)', color: '#FFF', padding: '0.7rem 4vw', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.2em' }}>
...
function NGONavbar({ settings, siteName, navPages, currentSlug, p, subdomain }: any) {
  return (
    <nav style={{ height: '100px', padding: '0 4vw', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.85)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(20px)', borderBottom: '1px solid #F1F5F9' }}>
      <Link to={`/preview/${subdomain}`} style={{ textDecoration: 'none', color: 'inherit', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
        {settings?.logo_url ? (
          ...
          <div style={{ display: 'flex', items: 'center', gap: '1.25rem' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '18px', background: 'linear-gradient(45deg, var(--primary), var(--secondary))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFF' }}>
              <Heart size={28} />
            </div>
            <span style={{ fontWeight: 900, fontSize: '1.8rem', color: 'var(--text-main)', letterSpacing: '-0.04em' }}>{siteName}</span>
          </div>
        )}
      </Link>
      <div style={{ display: 'flex', gap: '3vw', alignItems: 'center' }}>
        ...
        <Link to={`/preview/${subdomain}/contact`} style={{ padding: '0.8rem 2.5rem', background: 'var(--secondary)', color: '#FFF', borderRadius: '1.5rem', fontWeight: 900, fontSize: '0.8rem', textTransform: 'uppercase', textDecoration: 'none', boxShadow: '0 15px 30px -5px var(--secondary)33' }}>
           DONATE
        </Link>
      </div>
    </nav>
...
function NGOFooter({ settings, siteName, footerCfg, p }: any) {
      ...
      <div style={{ maxWidth: '1600px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1.5fr', gap: '8rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '2rem', marginBottom: '3.5rem' }}>
            <span style={{ fontWeight: 900, fontSize: '2rem', color: 'var(--text-main)' }}>{siteName}</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '2rem' }}>
             {(footerCfg.social_links || []).map((social: any, i: number) => (
                <a key={i} href={social.url} target="_blank" rel="noreferrer" style={{ color: 'var(--primary)', opacity: 0.3, transition: 'all 0.4s' }} onMouseOver={e => { e.currentTarget.style.opacity = '1'; e.currentTarget.style.transform = 'translateY(-5px) rotate(12deg)'; }} onMouseOut={e => { e.currentTarget.style.opacity = '0.3'; e.currentTarget.style.transform = 'none'; }}>
        ...
        <div>
           ...
           <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem', fontSize: '1.1rem' }}>
              {contact.phone && <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}><Phone size={20} color="var(--secondary)" /> <b>{contact.phone}</b></div>}
